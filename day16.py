...
def gt(x): return 1 if x[0] > x[1] else 0
def lt(x): return 1 if x[0] < x[1] else 0
def et(x): return 1 if x[0] == x[1] else 0
...
def decode_packets_num(packets, versions, number, operator=None):
    p_start, results = 0, []
    for _ in range(number):
        ...
        if p_type == 4:
            ...
            while packets[p_start] == "1":  # not last group
                ...
                p_start += 5
            value += packets[p_start+1:p_start+5]
            p_start += 5  # last group
            results.append(int(value, 2))
        else:
            if packets[p_start+6] == "0":  # length_type_id 0: 
                length = int(packets[p_start+7:p_start+22], 2)
                versions, result = decode_packets_len(packets[p_start+7+15:p_start+7+15+length], versions, length, p_type)
                p_start += 7 + 15 + length
            else:  # length_type_id 1
                number = int(packets[p_start+7:p_start+18], 2)
                versions, p_start_move, result = decode_packets_num(packets[p_start+18:], versions, number, p_type)
...
def puzzle1(answer=None):
    ...
    versions = []
    versions, *_ = decode_packets_num(bin_packet, versions, number=1)
...
def puzzle2(answer=None):
    ...
    bin_packet = bin(int(raw_input, 16))[2:].zfill(len(raw_input*4))
    versions = []
    versions, _, results = decode_packets_num(bin_packet, versions, number=1)
    print(results)
